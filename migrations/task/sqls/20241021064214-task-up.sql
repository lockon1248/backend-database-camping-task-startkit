...
SELECT * FROM "SKILL";
SELECT * FROM "SKILL"WHERE name = '空中瑜伽';
...
JOIN "COURSE" ON "COURSE_BOOKING".course_id = "COURSE".id
JOIN "USER" ON "COURSE_BOOKING".user_id = "USER".id
...
where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') 
and course_id = (select id from "COURSE" where user_id = (select id from "USER" where email = 'lee2000@hexschooltest.io'))
and status = '即將授課'; -- cancelled_at is null
...
where "COURSE_BOOKING".user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
  and status != '課程已取消' -- 為何是排除課程已取消的紀錄，而不是抓取已完成或上課中 ?
...
SELECT 
    cp.user_id,  -- 顯示用戶ID
    (cp.total_purchased - COALESCE(cb.total_used, 0)) as remaining_credit  
    -- 計算剩餘堂數 = 購買總數 - 已使用數
    -- COALESCE(cb.total_used, 0) 的意思是：如果 total_used 是空的，就用 0 代替
FROM (
    -- 第一個子查詢：計算王小明買了多少堂課
    SELECT 
        user_id,  -- 用戶ID
        SUM(purchased_credits) as total_purchased  -- 加總所有購買的堂數
    FROM "CREDIT_PURCHASE"  -- 從購買記錄表中查詢
    WHERE user_id = (
        SELECT id   -- 先找到王小明的用戶ID
        FROM "USER" 
        WHERE email = 'wXlTq@hexschooltest.io'
    )
    GROUP BY user_id  -- 按用戶分組（因為要加總）
) cp  -- 這個子查詢的結果我們叫它 cp

LEFT JOIN (  -- 使用 LEFT JOIN 是因為可能還沒上過課
    -- 第二個子查詢：計算王小明上了多少堂課
    SELECT 
        user_id,  -- 用戶ID
        COUNT(*) as total_used  -- 計算上課次數
    FROM "COURSE_BOOKING"  -- 從預約記錄表中查詢
    WHERE user_id = (
        SELECT id   -- 一樣先找到王小明的用戶ID
        FROM "USER" 
        WHERE email = 'wXlTq@hexschooltest.io'
    )
    AND status != '課程已取消'  -- 不計算取消的課
    GROUP BY user_id  -- 按用戶分組（因為要計數）
) cb ON cp.user_id = cb.user_id;  -- 這個子查詢的結果我們叫它 cb
...
limit 1; -- 使用排序 + limit 1 取得教練數量最多的專長
